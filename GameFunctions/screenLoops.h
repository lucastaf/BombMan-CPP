...
void MainGameLoop()
{
    ...
    for (int i = 0; i < QtdInimigos; i++)
    {
        if (inimigos[i].status)
            SumMapItens(currentframe, inimigos[i].inimigo); // Adiciona o inimigo se ele está vivo
    }
    if (bomba1.status)
        SumMapItens(currentframe, bomba1.bomba); // Adiciona a bomba se ela existe
    if (bomba1.status == 2)
        SumMapExplosion(currentframe, bomba1.explosao); // adiciona o raio da explosão se ela explodiu
    ...
    if (_kbhit())
    {
        ...
        case ' ': // Barra de espaço
            if (!bomba1.status)
                colocaBomba(player, bomba1); // Coloca a bomba se ela não existe
            break;
        }
    }
    //------------------
...
void mapEditorLoop()
{
    ...
    if (_kbhit())
    {
        ...
        case 'e': // seleciona o item anterior
            if (Iditemselecionado < QtdInimigos)
        ...
        case ' ': // Bara de espaço, coloca o item selecionado
            ...
            break;
        case 'z':
            m.mapa[ghost.y][ghost.x] = vazioid;
            break;
        case 'x':
            m.mapa[ghost.y][ghost.x] = paredeid;
            break;
        case 'c':
            m.mapa[ghost.y][ghost.x] = paredefragilid;
            break;
        case 27: // ESC
            gameRestart(player, inimigos, m, bomba1);
            system("cls");
    ...
    //------------------
    for (int i = 0; i <= QtdInimigos; i++)
    { // Qtdinimigos é o ultimo ID da array objetos
